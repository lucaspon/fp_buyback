...
describe("FingerprintsBuyback", function () {
    ...
    it("should allow the owner to trigger sendTokensToDestination, sending all ERC20 and NFTs to the destination address", async () => {
        ...
        await buybackContract.connect(deployer).sendTokensToDestination();
    ...
    it("should revert sendTokensToDestination when there aren't any ERC20s or ERC721s in the contract", async () => {
        ...
        await expect(buybackContract.connect(deployer).sendTokensToDestination()).to.be.revertedWith("No tokens or NFTs to send");
    });
    ...
    it("sendTokensToDestination should work if there aren't any ERC20 tokens but there are NFTs", async () => {
        // Mint an NFT and send it to the contract
        await mockERC721.mint();
        const tokenId = 0;
        await mockERC721.transferFrom(deployer.address, buybackContractAddress, tokenId);
        ...
        // Verify the contract holds no ERC20 tokens
        expect(await mockERC20.balanceOf(buybackContractAddress)).to.equal(0);
        ...
        // Verify the destination address holds the NFT
        expect(await mockERC721.ownerOf(tokenId)).to.equal(await buybackContract.DESTINATION_ADDRESS());
    });

    it("sendTokensToDestination should work if there aren't any NFTs but there are ERC20 tokens", async () => {
        // Mint some ERC20 tokens to the contract
        await mockERC20.transfer(buybackContractAddress, ethers.parseUnits("10000", 18));
        ...
        // Verify the contract holds no NFTs
        expect(await mockERC721.balanceOf(buybackContractAddress)).to.equal(0);

        // Send all ERC20 and NFTs to the destination address
        await buybackContract.connect(deployer).sendTokensToDestination();

        // Verify the destination address holds the ERC20 tokens
        expect(await mockERC20.balanceOf(await buybackContract.DESTINATION_ADDRESS())).to.equal(ethers.parseUnits("10000", 18));
    });

    it("should withdraw the entire ETH balance when amount is 0", async () => {
        const initialDeployerBalance = await ethers.provider.getBalance(deployer.address);
        const contractBalance = await ethers.provider.getBalance(buybackContractAddress);

        const withdrawTx = await buybackContract.connect(deployer).withdrawETH(0);
        const receipt = await withdrawTx.wait();
        ...
        const expectedFinalBalance = initialDeployerBalance + contractBalance - gasCost;

        const balanceDifference = finalDeployerBalance > expectedFinalBalance
            ? finalDeployerBalance - expectedFinalBalance
            : expectedFinalBalance - finalDeployerBalance;

        expect(balanceDifference <= ethers.parseEther("0.01")).to.be.true;
    });

    it("should enforce exchange rate boundaries", async () => {
        // Too low
        await expect(
            buybackContract.connect(deployer).setExchangeRateForOneMembership(ethers.parseEther("0.001"))
        ).to.be.revertedWith("Exchange rates must be greater than or equal to 0.01 ETH");

        // Too high
        await expect(
            buybackContract.connect(deployer).setExchangeRateForOneMembership(ethers.parseEther("1001"))
        ).to.be.revertedWith("Exchange rates must be less than or equal to 1000 ETH");

        // Same as current
        const currentRate = await buybackContract.nftExchangeRate();
        await expect(
            buybackContract.connect(deployer).setExchangeRateForOneMembership(currentRate)
        ).to.be.revertedWith("New exchange rates must be different from the current ones");
    });

    it("should not emit duplicate AddressAllowlisted events", async () => {
        await buybackContract.connect(deployer).addToAllowlist(user.address);
        await expect(buybackContract.connect(deployer).setAllowlist([user.address]))
            .to.not.emit(buybackContract, "AddressAllowlisted");
    });

    it("should emit ETHReceived on direct ETH transfer", async () => {
        await expect(
            deployer.sendTransaction({ to: buybackContractAddress, value: ethers.parseEther("1") })
        ).to.emit(buybackContract, "ETHReceived");
    });

    it("should emit ERC20Recovered when recovering tokens", async () => {
        await mockERC20.transfer(buybackContractAddress, ethers.parseUnits("1000", 18));
        await expect(
            buybackContract.connect(deployer).recoverERC20Tokens(mockERC20Address, ethers.parseUnits("1000", 18))
        ).to.emit(buybackContract, "ERC20Recovered")
            .withArgs(mockERC20Address, ethers.parseUnits("1000", 18));
    });

    it("should emit ERC721Recovered when recovering NFTs", async () => {
        await mockERC721.mint();
        await mockERC721.transferFrom(deployer.address, buybackContractAddress, 0);
        await expect(
            buybackContract.connect(deployer).recoverERC721Tokens(mockERC721Address, 0)
        ).to.emit(buybackContract, "ERC721Recovered")
            .withArgs(mockERC721Address, 0);
    });

    it("should emit TokensSentToDestination when only NFTs are present", async () => {
        await mockERC721.mint();
        await mockERC721.transferFrom(deployer.address, buybackContractAddress, 0);
        await expect(
            buybackContract.connect(deployer).sendTokensToDestination()
        ).to.emit(buybackContract, "TokensSentToDestination");
    });
