...
contract FingerprintsBuyback is ReentrancyGuard, Ownable, Pausable {
    ...
    event ERC721Recovered(address indexed token, uint256 indexed tokenId);
    ...
    function sendTokensToDestination() external nonReentrant onlyOwner {
        uint256 contractTokenBalance = token.balanceOf(address(this));
        uint256 nftBalance = nft.balanceOf(address(this));

        require(
            contractTokenBalance > 0 || nftBalance > 0,
    ...
    function withdrawETH(uint256 amount) external onlyOwner nonReentrant {
        // if amount provided is zero, withdraw the entire balance
        if (amount == 0) {
    ...
    function setAllowlist(address[] calldata _addresses) external onlyOwner {
        ...
        require(_addresses.length <= 500, "Batch size too large");

        for (uint256 i = 0; i < _addresses.length; i++) {
            if (_addresses[i] != address(0) && !allowlist[_addresses[i]]) {
