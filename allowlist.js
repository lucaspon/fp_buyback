...
const provider = new ethers.JsonRpcProvider(process.env.INFURA_URL || 'https://mainnet.infura.io/v3/YOUR_INFURA_KEY');
    ...
    for (const addr of addressArray) {
        try {
            if (ethers.isAddress(addr)) {
                ...
                else console.error(`Could not resolve ENS name: ${addr}`);
            }
        } catch (error) {
